$(function() {
    function t(t, e) {
        ...
        var n = d3.select(t),
            ...
            u = d3.time.scale().range([0, s]),
            p = d3.scale.linear().range([e, 0]),
            f = d3.scale.ordinal().range(c),
            m = d3.svg.axis().scale(u).orient("bottom").ticks(d3.time.hours, 4).innerTickSize(4).tickPadding(8).tickFormat(d3.time.format("%H:%M")),
            h = d3.svg.axis().scale(p).ticks(6).innerTickSize(4).outerTickSize(0).tickPadding(8).tickFormat(function(t) {
                return t / 1e3 + "k"
            }),
            g = h,
            y = d3.svg.axis().scale(p).orient("left").ticks(6).tickPadding(8).tickFormat("").tickSize(-s, 0, 0),
            v = n.append("svg"),
            x = v.attr("width", s + r.left + r.right).attr("height", e + r.top + r.bottom).append("g").attr("transform", "translate(" + r.left + "," + r.top + ")"),
        ...
        d3.csv("assets/demo_data/dashboard/traffic_sources.csv", function(a, c) {
            ...
            var A = x.append("g").attr("class", "streamgraph-layers-group"),
                M = A.selectAll(".streamgraph-layer").data(v).enter().append("path").attr("class", "streamgraph-layer").attr("d", function(t) {
                    return k(t.values)
                }).style("stroke", "#fff").style("stroke-width", .5).style("fill", function(t, e) {
                    return f(e)
                }),
                C = M.style("opacity", 0).transition().duration(750).delay(function(t, e) {
                    return 50 * e
                }).style("opacity", 1);
            x.append("g").attr("class", "d3-axis d3-axis-left d3-axis-solid").call(h.orient("left")), d3.select(x.selectAll(".d3-axis-left .tick text")[0][0]).style("visibility", "hidden"), x.append("g").attr("class", "d3-axis d3-axis-right d3-axis-solid").attr("transform", "translate(" + s + ", 0)").call(g.orient("right")), d3.select(x.selectAll(".d3-axis-right .tick text")[0][0]).style("visibility", "hidden");
            var z = x.append("g").attr("class", "d3-axis d3-axis-horizontal d3-axis-solid").attr("transform", "translate(0," + e + ")").call(m);
            z.selectAll(".d3-axis-subticks").data(u.ticks(d3.time.hours), function(t) {
                return t
            ...
            var B = A.append("g").attr("class", "hover-line"),
                ...
                R = B.append("rect").attr("x", 2).attr("y", 2).attr("width", 6).attr("height", 6).style("fill", "#03A9F4").style("stroke", "#fff").style("stroke-width", 1).style("shape-rendering", "crispEdges").style("pointer-events", "none").style("opacity", 0);
            C.each("end", function() {
                M.on("mouseover", function(t, e) {
                    ...
                    mousedate = datearray.indexOf(a), pro = t.values[mousedate].value, R.attr("x", mousex - 3).attr("y", mousey - 6).style("opacity", 1), F.attr("x1", mousex).attr("x2", mousex).style("opacity", 1), l.html("<ul class='list-unstyled mb-5'><li><div class='text-size-base mt-5 mb-5'><i class='icon-circle-left2 position-left'></i>" + t.key + "</div></li><li>Visits: &nbsp;<span class='text-semibold pull-right'>" + pro + "</span></li><li>Time: &nbsp; <span class='text-semibold pull-right'>" + d(t.values[mousedate].date) + "</span></li></ul>").style("display", "block"), l.append("div").attr("class", "d3-tip-arrow")
                }).on("mouseout", function(t, e) {
                    x.selectAll(".streamgraph-layer").transition().duration(250).style("opacity", 1), R.style("opacity", 0), l.style("display", "none"), F.style("opacity", 0)
                })
            }), n.on("mousemove", function(e, a) {
                mouse = d3.mouse(this), mousex = mouse[0], mousey = mouse[1], l.style("top", mousey - $(".d3-tip").outerHeight() / 2 - 2 + "px"), mousex >= $(t).outerWidth() - $(".d3-tip").outerWidth() - r.right - 2 * i ? l.style("left", mousex - $(".d3-tip").outerWidth() - i + "px").attr("class", "d3-tip w") : l.style("left", mousex + i + "px").attr("class", "d3-tip e")
            })
    ...
    function e(t, e) {
        function a() {
            d3.transition().duration(m ? 7500 : 500).each(n)
        }

        function n() {
            function t() {
                i = r.node().getBoundingClientRect().width - s.left - s.right, u.attr("width", i + s.left + s.right), p.attr("width", i + s.left + s.right), g.range([0, i]), y.range([e, 0]), p.select(".d3-axis-horizontal").call(v), p.select(".d3-axis-vertical").call(x.tickSize(0 - i)), p.selectAll(".d3-line").attr("d", function(t, e) {
                    return h(t.values)
            ...
            var m = c.domain().map(function(t) {
                ...
                g = d3.time.scale().domain([d3.min(m, function(t) {
                ...
                y = d3.scale.linear().domain([d3.min(m, function(t) {
                    return d3.min(t.values, function(t) {
                        return t.value
                    })
                }), d3.max(m, function(t) {
                ...
                x = d3.svg.axis().scale(y).orient("left").ticks(6).tickSize(0 - i).tickPadding(8);
            ...
            var b = p.selectAll(".lines").data(m),
    ...
    function a(t, e, a) {
        var n = d3.select(t),
            ...
            m = d3.svg.axis().scale(p).orient("bottom").ticks(d3.time.days, 6).innerTickSize(4).tickPadding(8).tickFormat(d3.time.format("%b %d"));
        d3.json("assets/demo_data/dashboard/monthly_sales.json", function(t, h) {
            ...
            function y() {
                s = n.node().getBoundingClientRect().width - r.left - r.right, d.attr("width", s + r.left + r.right), c.attr("width", s + r.left + r.right), p.range([0, s]), c.selectAll(".d3-axis-horizontal").call(m), c.selectAll(".d3-axis-subticks").attr("x1", p).attr("x2", p), c.selectAll(".d3-area").datum(h).attr("d", u), c.selectAll(".d3-crosshair-overlay").attr("width", s)
            }
            if (t) return console.error(t);
            h.forEach(function(t) {
                t.date = i(t.date), t.value = +t.value
            });
            var v = d3.max(h, function(t) {
                    return t.value
                }),
                x = h.map(function(t) {
                    return {
            ...
            p.domain(d3.extent(h, function(t, e) {
                return t.date
            })), f.domain([0, d3.max(h, function(t) {
                return t.value
            })]);
            var b = c.append("g").attr("class", "d3-axis d3-axis-horizontal d3-axis-solid").attr("transform", "translate(0," + e + ")").call(m);
            b.selectAll(".d3-axis-subticks").data(p.ticks(d3.time.days), function(t) {
                return t
            }).enter().append("line").attr("class", "d3-axis-subticks").attr("y1", 0).attr("y2", 4).attr("x1", p).attr("x2", p), c.append("path").datum(h).attr("class", "d3-area").attr("d", u).style("fill", a).transition().duration(1e3).attrTween("d", function() {
                var t = d3.interpolateArray(x, h);
                return function(e) {
                    return u(t(e))
                }
            });
            ...
            A.append("text").attr("dy", -10).style("font-size", 12), c.append("rect").attr("class", "d3-crosshair-overlay").style("fill", "none").style("pointer-events", "all").attr("width", s).attr("height", e).on("mouseover", function() {
    ...
    function n(t, e, a) {
        ...
        d3.json("assets/demo_data/dashboard/monthly_sales.json", function(t, e) {
            function p() {
                s = n.node().getBoundingClientRect().width - r.left - r.right, l.attr("width", s + r.left + r.right), o.attr("width", s + r.left + r.right), c.range([0, s]), o.selectAll(".d3-area").datum(e).attr("d", d)
            }
            if (t) return console.error(t);
    ...
    function r(t, e, a, n, r, s, i, l) {
        function o() {
            A.attr("transform", null).transition().duration(s).ease("linear").attr("transform", "translate(" + h(0) + ",0)"), "area" == e ? A.attr("d", v).attr("class", "d3-area").style("fill", l) : A.attr("d", y).attr("class", "d3-line d3-line-medium").style("stroke", l)
        }

        function d() {
            p = c.node().getBoundingClientRect().width - u.left - u.right, x.attr("width", p + u.left + u.right), b.attr("width", p + u.left + u.right), h.range([0, p]), k.attr("width", p), b.select(".d3-line").attr("d", y), b.select(".d3-area").attr("d", v)
        }
        for (var c = d3.select(t), u = {
                ...
                bottom: 0,
                left: 0
            }, p = c.node().getBoundingClientRect().width - u.left - u.right, n = n - u.top - u.bottom, f = [], m = 0; a > m; m++) f.push(Math.floor(Math.random() * a) + 5);
        var h = d3.scale.linear().range([0, p]),
            g = d3.scale.linear().range([n - 5, 5]);
        h.domain([1, a - 3]), g.domain([0, a]);
        ...
        }).append("path").datum(f).attr("transform", "translate(" + h(0) + ",0)");
        ...
        }, i), $(window).on("resize", d), $(".sidebar-control").on("click", d)
    }

    function s(t, e) {
        function a() {
            i = r.node().getBoundingClientRect().width - s.left - s.right, u.attr("width", i + s.left + s.right), p.attr("width", i + s.left + s.right), f.range([l, i - l]), y.attr("width", i), p.selectAll(".d3-line").attr("d", h(n)), p.selectAll(".d3-line-circle").attr("cx", h.x()), p.selectAll(".d3-line-guides").attr("x1", function(t, e) {
                return f(t.date)
        ...
        var f = d3.time.scale().range([l, i - l]),
            m = d3.scale.linear().range([e, 5]);
        f.domain(d3.extent(n, function(t) {
            return t.date
        })), m.domain([0, d3.max(n, function(t) {
            return Math.max(t.alpha)
        })]);
        var h = d3.svg.line().x(function(t) {
                return f(t.date)
            }).y(function(t) {
                return m(t.alpha)
            }),
        ...
        y.transition().duration(1e3).ease("linear").attr("width", i);
        p.append("path").attr({
            d: h(n),
        ...
        var x = p.insert("g").selectAll(".d3-line-circle").data(n).enter().append("circle").attr("class", "d3-line-circle d3-line-circle-medium").attr("cx", h.x()).attr("cy", h.y()).attr("r", 3).style("stroke", "#fff").style("fill", "#607D8B");
    ...
    function i(t, e, a, n) {
        ...
        var m = f.append("path").style("fill", n);
        m.transition().ease("cubic-out").duration(2500).attrTween("d", r)
    }
    ...
    function l(t, e) {
        var a = [{
            ...
            u = d3.svg.arc().outerRadius(s).innerRadius(s / 2),
    ...
    function c(t, e, a, n, r, s, i, l, o) {
        ...
        function u() {
            h = m.node().getBoundingClientRect().width, v.attr("width", h), x.attr("width", h), g.rangeBands([0, h], .3), x.selectAll(".d3-random-bars").attr("width", g.rangeBand()).attr("x", function(t, e) {
                return g(e)
            })
        }
        for (var p = [], f = 0; e > f; f++) p.push(Math.round(10 * Math.random()) + 10);
        var m = d3.select(t),
            h = m.node().getBoundingClientRect().width,
            g = d3.scale.ordinal().rangeBands([0, h], .3),
            y = d3.scale.linear().range([0, a]);
        g.domain(d3.range(0, p.length)), y.domain([0, d3.max(p)]);
        var v = m.append("svg"),
            x = v.attr("width", h).attr("height", a).append("g"),
    ...
    function u(t, e, a, n, r, s, i, l) {
        function o(t) {
            w.attr("d", b.endAngle(f * t)), k.attr("d", b.endAngle(f * t)), A.text(m(t))
        }
        var d = d3.select(t),
            c = 0,
            u = 32,
            ...
            y = c > p ? -.01 : .01,
            v = d.append("svg"),
            x = v.attr("width", h).attr("height", h).append("g").attr("transform", "translate(" + h / 2 + "," + h / 2 + ")"),
        ...
        d3.select(t).append("i").attr("class", s + " counter-icon").attr("style", "top: " + (h - u) / 2 + "px"), d3.select(t).append("div").text(i), d3.select(t).append("div").attr("class", "text-size-small text-muted").text(l);
        var M = c;
        ! function C() {
            o(M), g > 0 && (g--, M += y, setTimeout(C, 10))
        }()
    }
    var p = Array.prototype.slice.call(document.querySelectorAll(".switch"));
    p.forEach(function(t) {
        ...
        }), $(".daterange-ranges span").html(moment().subtract("days", 29).format("MMMM D") + " - " + moment().format("MMMM D")), t("#traffic-sources", 330), e("#app_sales", 255), a("#monthly-sales-stats", 100, "#4DB6AC"), n("#messages-stats", 40, "#5C6BC0"), r("#new-visitors", "line", 30, 35, "basis", 750, 2e3, "#26A69A"), r("#new-sessions", "line", 30, 35, "basis", 750, 2e3, "#FF7043"), r("#total-online", "line", 30, 35, "basis", 750, 2e3, "#5C6BC0"), r("#server-load", "area", 30, 50, "basis", 750, 2e3, "rgba(255,255,255,0.5)"), s("#today-revenue", 50), i("#today-progress", 20, 20, "#7986CB"), i("#yesterday-progress", 20, 20, "#7986CB"), l("#campaigns-donut", 42), o("#campaign-status-pie", 42), d("#tickets-status", 42), c("#hours-available-bars", 24, 40, !0, "elastic", 1200, 50, "#EC407A", "hours"), c("#goal-bars", 24, 40, !0, "elastic", 1200, 50, "#5C6BC0", "goal"), c("#members-online", 24, 50, !0, "elastic", 1200, 50, "rgba(255,255,255,0.5)", "members"), u("#hours-available-progress", 38, 2, "#F06292", .68, "icon-watch text-pink-400", "Hours available", "64% average"),
        u("#goal-progress", 38, 2, "#5C6BC0", .82, "icon-trophy3 text-indigo-400", "Productivity goal", "87% average"), $(".table tr").each(function(t) {
